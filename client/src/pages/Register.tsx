...
const SOURCE = {
  heading: 'Sukurti paskyrą',
  subtitle: 'Prisijunk prie tūkstančių besiruošiančių egzaminui',
  nameLabel: 'Vardas ir pavardė',
  emailLabel: 'El. pašto adresas',
  passwordLabel: 'Slaptažodis',
  terms: 'Registruodamiesi sutinkate su mūsų naudojimo sąlygomis.',
  submitBtn: 'Registruotis nemokamai',
...
export default function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    setError('')
    if (password.length < 6) { setError('Slaptažodis turi būti bent 6 simboliai'); return }
    setLoading(true)
    try {
      ...
      setError(msg || 'Registracijos klaida. Bandykite dar kartą.')
    } finally {
      setLoading(false)
    }
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} required
                className="input-field" placeholder="Jonas Jonaitis" />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required
                  className="input-field pr-11" placeholder="Bent 6 simboliai" />
